...
Insight is configured through two files: a `.env` file for API keys and a `data/config.json` file for sources, AI provider, and filtering options.
...
When Insight starts, environment variables have priority because
`data/config.json` does not store the secret. For local VS Code runs, create
`.env` in the repository root and launch Insight from that same root directory.
...
`INSIGHT_OLLAMA_BASE_URL` in `.env`. `OLLAMA_BASE_URL` and `OLLAMA_HOST` are
also recognized. If the value omits `/v1`, Insight appends it automatically
...
For OpenAI-compatible gateways, Insight sends `temperature` by default. If a newer reasoning-style model rejects that parameter with an error such as `temperature is deprecated for this model`, Insight retries once without it and remembers that capability for later requests.
...
- `name` — label shown in Insight metadata and selection breakdowns
...
OpenBB provider credentials are handled by the OpenBB SDK itself, using its own environment variables or user settings. Insight does not pass those secrets through `data/config.json`.
...
If the same category appears in multiple groups, Insight logs a warning and uses
...
```json
{
  ...
  "webhook": {
    "url_env": "INSIGHT_WEBHOOK_URL",
    "headers": "Authorization: Bearer ${INSIGHT_WEBHOOK_TOKEN}"
  }
}
```

- `${NAME}` is replaced only when `NAME` is a valid identifier like `LWN_KEY` or `INSIGHT_AI_BASE_URL`.
...
Email delivery is optional and disabled unless `email.enabled` is `true`. Insight uses SMTP to send daily summaries and IMAP to check subscribe/unsubscribe requests.
...
- `smtp_username`: Optional SMTP login username. If omitted, Insight uses `email_address`.
...
- `subscribe_keyword` / `unsubscribe_keyword`: Keywords Insight looks for in incoming email subjects.
...
Webhook notification is optional and disabled unless `webhook.enabled` is `true`. Insight can call Feishu/Lark, DingTalk, Slack, Discord, or any custom webhook endpoint when the pipeline succeeds or fails.
...
- `url_env`: Environment variable that contains the webhook URL. For example, set `INSIGHT_WEBHOOK_URL=https://...` in `.env`.
...
- `request_body`: Optional request body. If empty, Insight sends a `GET` request. If provided, Insight sends a `POST` request.
- `headers`: Optional custom headers, one `Key: Value` pair per line.

When `request_body` is a JSON object or array, Insight renders placeholders and serializes it as JSON. When it is a string, Insight renders it directly and detects JSON if the rendered string is valid JSON.

### Delivery Modes And Layouts

`delivery` controls how many webhook messages Insight sends:
...
- `collapsible`: Currently supported for `platform: "feishu"` or `"lark"`. Insight ignores `request_body` and builds one Feishu/Lark Card JSON 2.0 message with each item in a collapsed panel.
...
With `summary_and_items`, Insight sends one overview plus one message per selected item. `overview_position: "last"` sends item messages first and keeps the overview as the newest chat message; omit it or set `"first"` to send the overview first.
...
For webhook delivery, Insight flattens HTML disclosure blocks such as `<details><summary>...</summary>` in `#{summary}` into plain Markdown link lists. This makes the generated summary easier to render in chat products. Saved Markdown files, GitHub Pages, and email content are unchanged.
...
In DingTalk, create a custom group robot and use a custom keyword such as `Insight`. The keyword must appear in the body content.

```json
{
  "msgtype": "markdown",
  "markdown": {
    "title": "Insight #{date} Daily",
    "text": "Insight result: #{result}\n\nInsight important items: #{important_items}/#{all_items}\n\n#{summary}"
  }
}
```
...
In Feishu or Lark, create a custom group robot and use a custom keyword such as `Insight`. The keyword must appear in the body content.
...
With this layout, Insight sends one interactive card containing the overview and one collapsed panel per selected item. Each panel can be expanded in Feishu to read the full item detail. The regular `request_body` template is ignored for this rendered card.
...
Insight writes generated summaries to `data/summaries/` and copies publishable Markdown into `docs/` for the GitHub Pages site. The repository includes a ready-to-use workflow at `.github/workflows/daily-summary.yml`.
...
Insight includes an MCP server for AI assistants and MCP-compatible clients.
