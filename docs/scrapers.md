...
Insight fetches content from multiple source types. All scrapers inherit from `BaseScraper`, share an async HTTP client, and implement a `fetch(since)` method that returns a list of `ContentItem` objects. Sources are fetched concurrently via `asyncio.gather`.
...
The scraper imports `openbb` lazily. If the optional dependency is not installed, Insight logs a warning and skips the source instead of failing the whole run.
...
**Credentials**: provider-specific secrets are resolved by the OpenBB SDK from its own environment variables or settings file. Insight does not pass those values directly.
