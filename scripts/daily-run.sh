#!/usr/bin/env bash
# Insight daily run + deploy to GitHub Pages
# Usage: ./scripts/daily-run.sh
# Cron:  0 8 * * * /path/to/insight/scripts/daily-run.sh >> /path/to/insight/logs/cron.log 2>&1
...
echo "$LOG_PREFIX Starting Insight daily run..."
