...
<p><strong>你只需享受新闻，剩下交给 Insight。</strong></p>

<a href="https://trendshift.io/repositories/22864?utm_source=trendshift-badge&amp;utm_medium=badge&amp;utm_campaign=badge-trendshift-22864" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/trendshift/repositories/22864/daily" alt="Thysrael%2FInsight | Trendshift" width="250" height="55"/></a>
<a href="https://trendshift.io/repositories/22864?utm_source=trendshift-badge&amp;utm_medium=badge&amp;utm_campaign=badge-trendshift-22864" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/trendshift/repositories/22864/weekly?language=Python" alt="Thysrael%2FInsight | Trendshift" width="250" height="55"/></a>
<a href="https://hellogithub.com/repository/Thysrael/Insight" target="_blank"><img src="https://abroad.hellogithub.com/v1/widgets/recommend.svg?rid=7a4b606e28e4477998d35851cf4fdddf&claim_uid=rtjnLkYT7ziQJUG" alt="Featured｜HelloGitHub" style="width: 250px; height: 54px;" width="250" height="54" /></a>
<br>
...
[![Website](https://img.shields.io/badge/Website-Insight-263238?style=for-the-badge&logo=homepage&logoColor=white)](https://www.insight1123.top/)
[![Daily](https://img.shields.io/github/actions/workflow/status/Thysrael/Insight/deploy-docs.yml?branch=main&label=Daily&style=for-the-badge&logo=date-fns&logoColor=white)](https://thysrael.github.io/Insight/)
[![Commit](https://img.shields.io/github/commit-activity/m/Thysrael/Insight?label=Commit&style=for-the-badge&logo=github&logoColor=white)](https://github.com/Thysrael/Insight/commits/main)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Thysrael/Insight/pulls)
...
[📖 在线演示](https://thysrael.github.io/Insight/) · [📋 配置指南](https://thysrael.github.io/Insight/configuration) · [English](README.md) · [日本語](README_ja.md)
...
好新闻分散在各处，坏信息却源源不断。Insight 为你先完成第一轮筛选：从 Hacker News、Reddit、Telegram、RSS、Twitter/X、GitHub 和 OpenBB 抓取内容，合并重复新闻，用 AI 打分过滤，并为重要内容补充背景解释和社区讨论。

但 Insight 不只是又一个摘要工具。AI 很擅长降低噪声，但新闻仍然需要人的品味：你信任哪些信息源，哪些评论改变了你对事件的理解，哪些小众来源值得被更多人看见。Insight 通过可定制的信息源、筛选标准、模型、语言、分发方式、评论摘要和社区信息源官网，把这层“人味”保留下来。
...
Insight 是一个业余时间维护的开源项目。如果你愿意支持这个项目，或希望出现在这里，欢迎[创建一个 Issue](https://github.com/Thysrael/Insight/issues/new) 或[发邮件](mailto:thysrael@163.com)联系我。

| 支持方 | 说明 |
|--------|------|
| [<img src="docs/assets/compshare-logo.png" alt="Compshare / 优云智算" width="220" />](https://www.compshare.cn/?ytag=GPU_YY_git_Insight) | 优云智算目前正在支持 Insight。优云智算是 UCloud 旗下 AI 云平台，主打包月、按次的高性价比国模 Agent Plan 套餐，低至 49 元/月起，同时提供官转稳定海外模型。支持接入 Claude Code、Codex 及 API 调用，支持企业高并发、7*24 技术支持和自助开票。<br><br>通过其[链接](https://www.compshare.cn/?ytag=GPU_YY_git_Insight)注册，可获得 5 元平台体验金。 |
...
```bash
git clone https://github.com/Thysrael/Insight.git
cd insight
...
docker compose run --rm insight

# 或自定义时间窗口
docker compose run --rm insight --hours 48
```
...
```bash
uv run insight-wizard
```

向导会询问你的兴趣（如"LLM 推理"、"嵌入式"、"web 安全"），自动推荐并生成 `data/config.json`，还可选让 AI 补充推荐小众源。若你想分享信息源，请前往 [insight1123.top](https://insight1123.top/)。
...
```bash
uv run insight              # 使用默认 24 小时窗口
uv run insight --hours 48   # 抓取最近 48 小时的内容
```

#### 使用 Docker

```bash
docker compose run --rm insight              # 使用默认 24 小时窗口
docker compose run --rm insight --hours 48   # 抓取最近 48 小时的内容
```
...
Insight 非常适合作为 **GitHub Actions** 定时任务运行。查看 [`.github/workflows/daily-summary.yml`](.github/workflows/daily-summary.yml) 获取现成的工作流配置，可自动生成日报并部署到 GitHub Pages。
...
| [评分机制](docs/scoring.md) | Insight 如何评估和排序新闻 |
| [抓取器](docs/scrapers.md) | 信息源抓取器说明和扩展细节 |
| [MCP 工具](src/mcp/README.md) | MCP 客户端可调用的工具说明 |

## 项目状态

Insight 已经支持完整的日报流程：多源抓取、AI 打分、去重、背景补充、评论摘要、双语生成、GitHub Pages 发布、邮件分发、Webhook 推送、Docker 部署、MCP 集成和配置向导。
...
想把有价值的信息源分享给 Insight 社区？请直接前往 **[insight1123.top](https://insight1123.top)** 提交。
