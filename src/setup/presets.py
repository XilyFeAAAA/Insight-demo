...
Supports loading from the insight-site API (preferred) or local file (fallback).
API data is served in insight-site's own format (Category/SourceType enums)
...
API_BASE_URL = os.environ.get("INSIGHT_API_URL", "https://insight1123.top")
PRESETS_ENDPOINT = f"{API_BASE_URL}/api/presets"
REQUEST_TIMEOUT = 10  # seconds


def fetch_presets() -> Optional[Dict]:
    """Fetch presets from the insight-site API.
...
def _transform_api_response(api_data: Dict) -> Dict:
    """Transform insight-site API response to the internal preset format.

    The API returns data in insight-site's format (Category enum IDs like
    ...
    for category in api_data.get("categories", []):
        ...
        for src in category.get("sources", []):
            ...
            # Insight-site stores "name" at the source level, but
            # insight's build_config() expects it inside config for RSS sources.
            src_type = src.get("type", "rss")
            source_name = src.get("name", "")
            if src_type == "rss" and source_name and "name" not in config:
                config["name"] = source_name

            # Remove the internal "subtype" field that insight-site uses
            # for GitHub sources — insight uses the type field instead.
...
def load_presets(
    ...
    Args:
        ...
        prefer_api: Whether to try the API first. Set False for offline mode
            or when INSIGHT_OFFLINE environment variable is set.
    ...
    """
    offline = os.environ.get("INSIGHT_OFFLINE", "").lower() in ("1", "true", "yes")
