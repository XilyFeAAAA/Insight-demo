"""MCP server entrypoint for Insight."""
...
from .service import InsightPipelineService


mcp = FastMCP(name="insight-mcp")
...
def _err(
    ...
    if isinstance(error, InsightMcpError):
        code = error.code
...
async def hz_validate_config(
    ...
    """Validate Insight config and required environment variables."""
...
async def hz_generate_summary(
    ...
    insight_path: str | None = None,
    config_path: str | None = None,
    save_to_insight_data: bool = False,
) -> dict[str, Any]:
    ...
    return await _run_tool(
        ...
        lambda: service.generate_summary(
            ...
            insight_path=insight_path,
            config_path=config_path,
            save_to_insight_data=save_to_insight_data,
        ),
    )
...
async def hz_send_webhook(
    ...
    and headers from the Insight config. Template variables #{date}, #{language},
...
@mcp.resource("insight://server/info")
...
@mcp.resource("insight://metrics")
def r_metrics() -> dict[str, Any]:
    """In-memory metrics snapshot."""

    return _resource_result("insight://metrics", _metrics_snapshot)


@mcp.resource("insight://runs")
def r_runs() -> dict[str, Any]:
    """Recent run list."""

    return _resource_result("insight://runs", lambda: service.list_runs(limit=30))


@mcp.resource("insight://runs/{run_id}/meta")
def r_run_meta(run_id: str) -> dict[str, Any]:
    """Run metadata resource."""

    return _resource_result(
        f"insight://runs/{run_id}/meta",
        lambda: service.get_run_meta(run_id),
    )


@mcp.resource("insight://runs/{run_id}/items/{stage}")
def r_run_items(run_id: str, stage: str) -> dict[str, Any]:
    """Run stage items resource."""

    return _resource_result(
        f"insight://runs/{run_id}/items/{stage}",
        lambda: service.get_run_stage(run_id=run_id, stage=stage, max_items=200),
    )


@mcp.resource("insight://runs/{run_id}/summary/{language}")
def r_run_summary(run_id: str, language: str) -> dict[str, Any]:
    """Run summary resource."""

    return _resource_result(
        f"insight://runs/{run_id}/summary/{language}",
        lambda: service.get_run_summary(run_id=run_id, language=language),
    )


@mcp.resource("insight://config/effective")
def r_effective_config() -> dict[str, Any]:
    """Effective default config resolved from local Insight path."""

    return _resource_result("insight://config/effective", service.get_effective_config)
