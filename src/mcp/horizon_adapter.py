"""Adapter layer that reuses Insight's native Python modules."""
...
from .errors import InsightMcpError
...
class InsightRuntime:
    """Loaded runtime references from a Insight codebase."""

    insight_path: Path
    ContentItem: Any
    Config: Any
    StorageManager: Any
    InsightOrchestrator: Any
...
def resolve_insight_path(explicit: str | None = None) -> Path:
    """Resolve Insight repository path by explicit arg/env/common locations."""

    candidates: list[Path] = []
    if explicit:
        candidates.append(Path(explicit).expanduser())

    env_path = os.getenv("INSIGHT_PATH")
    if env_path:
    ...
    for candidate in candidates:
        ...
        if path in seen:
            continue
        seen.add(path)
        if _is_insight_repo(path):
            return path

    checked = ", ".join(str(p.resolve()) for p in candidates)
    raise InsightMcpError(
        code="HZ_INSIGHT_NOT_FOUND",
        message="Insight repository was not found. Pass insight_path or set INSIGHT_PATH.",
        details={"checked": checked},
    )


def resolve_config_path(insight_path: Path, config_path: str | None = None) -> Path:
    """Resolve config path, defaulting to <insight>/data/config.json."""

    if not config_path:
        path = (insight_path / "data/config.json").resolve()
    else:
        raw = Path(config_path).expanduser()
        if raw.is_absolute():
            path = raw.resolve()
        else:
            candidate = (insight_path / raw).resolve()
...
def load_runtime(insight_path: Path) -> InsightRuntime:
    """Load Insight modules dynamically from local repository path."""

    if not _is_insight_repo(insight_path):
        raise InsightMcpError(
            code="HZ_INVALID_INSIGHT_PATH",
            message="insight_path is not a valid Insight repository.",
            details={"insight_path": str(insight_path)},
        )

    load_dotenv(insight_path / ".env", override=False)
    _load_mcp_secrets(insight_path, override=False)

    insight_path_str = str(insight_path)
    if insight_path_str not in sys.path:
        sys.path.insert(0, insight_path_str)
    ...
    try:
        ...
        raise InsightMcpError(
            ...
            message="Failed to load Insight modules.",
    ...
    return InsightRuntime(
        ...
        InsightOrchestrator=orchestrator.InsightOrchestrator,
...
def load_config(runtime: InsightRuntime, config_path: Path) -> Any:
    """Load Insight config using native pydantic model."""
...
def make_storage(runtime: InsightRuntime, config_path: Path) -> Any:
    """Build Insight storage manager bound to config's data directory."""

    data_dir = str(config_path.parent.resolve())
    return runtime.StorageManager(data_dir=data_dir)


def make_orchestrator(runtime: InsightRuntime, config: Any, storage: Any) -> Any:
    """Build native Insight orchestrator."""

    return runtime.InsightOrchestrator(config, storage)
...
def items_to_dicts(items: list[Any]) -> list[dict[str, Any]]:
    """Serialize Insight ContentItem models."""

    return [item.model_dump(mode="json") for item in items]


def dicts_to_items(runtime: InsightRuntime, payload: list[dict[str, Any]]) -> list[Any]:
...
def _is_insight_repo(path: Path) -> bool:
    return (path / "src" / "main.py").exists() and (path / "pyproject.toml").exists()


def _load_mcp_secrets(insight_path: Path, override: bool = False) -> None:
    """Load MCP secrets from JSON and inject string environment variables."""

    secrets_path = _resolve_secrets_path(insight_path)
    if not secrets_path:
        return
...
def _resolve_secrets_path(insight_path: Path) -> Path | None:
    """Resolve secrets config path via env and common locations."""

    explicit = os.getenv("INSIGHT_MCP_SECRETS_PATH")
    if explicit:
        ...
        raise InsightMcpError(
            ...
            message="INSIGHT_MCP_SECRETS_PATH points to a missing file.",
    ...
    candidates = [
        ...
        insight_path / "data" / "mcp.secrets.json",
        insight_path / "data" / "mcp-secrets.json",
    ]
    for candidate in candidates:
