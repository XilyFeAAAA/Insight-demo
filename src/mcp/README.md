...
Insight includes a built-in MCP server that exposes the native Insight pipeline as staged tools and read-only resources.

The MCP layer does not reimplement Insight business logic. It reuses the existing fetch, score, filter, enrich, and summarize modules from the main codebase.

## Tools

| Tool | Description |
| --- | --- |
| `hz_validate_config` | Validate Insight config and required environment variables |
...
- `insight://metrics`
- `insight://runs`
- `insight://runs/{run_id}/meta`
- `insight://runs/{run_id}/items/{stage}`
- `insight://runs/{run_id}/summary/{language}`
- `insight://config/effective`
...
1. Keep Insight as the single source of business logic.
