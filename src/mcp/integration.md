# Insight MCP Integration

## Recommended Command

Start the built-in MCP server from the Insight repository root:
...
Some MCP clients need a fixed working directory in their config. In that case, the absolute path is only used in the client-side `cwd` field, not in Insight's code.
...
```json
{
  "mcpServers": {
    "insight": {
      "command": "uv",
      "args": ["run", "insight-mcp"],
      "cwd": "/absolute/path/to/Insight"
    }
  }
}
```
...
```json
{
  "INSIGHT_MCP_SECRETS_PATH": "/absolute/path/to/mcp.secrets.json"
}
```
