"""Application service for staged Insight pipeline execution."""
...
class InsightPipelineService:
    ...
    def get_effective_config(
        ...
        return {
            "insight_path": str(ctx.insight_path),
    ...
    def _build_context(
        ...
        resolved_insight = resolve_insight_path(insight_path)
        runtime = load_runtime(resolved_insight)
        resolved_config = resolve_config_path(resolved_insight, config_path)
    ...
    def _load_stage_items(
        ...
        ctx, _, _ = self._build_context(
            insight_path=insight_path, config_path=config_path, sources=None
        )
        try:
    ...
    async def send_webhook(
        ...
        variables = {
            ...
            "message_title": f"Insight {date} webhook",
            "message_kind": "manual",
            "summary": summary,
        }
