...
from src.orchestrator import InsightOrchestrator
...
def make_orchestrator(filtering: FilteringConfig) -> InsightOrchestrator:
    orchestrator = InsightOrchestrator.__new__(InsightOrchestrator)
...
def test_run_applies_balanced_digest_before_enrichment(tmp_path, monkeypatch) -> None:
    ...
    orchestrator = InsightOrchestrator(config, storage)
    items = [
        make_item("ai", 9.0, "ai"),
        make_item("finance", 8.0, "finance"),
