...
from src.mcp.insight_adapter import (
    _load_mcp_secrets,
    apply_source_filter,
    load_config,
    load_runtime,
...
def test_resolve_insight_path_accepts_explicit_repo() -> None:
    repo_root = Path(__file__).resolve().parents[1]

    assert resolve_insight_path(str(repo_root)) == repo_root.resolve()
...
def test_load_mcp_secrets_loads_generic_env_keys(tmp_path: Path, monkeypatch) -> None:
    ...
    repo_root = Path(__file__).resolve().parents[1]
    monkeypatch.setenv("INSIGHT_MCP_SECRETS_PATH", str(secrets_path))
